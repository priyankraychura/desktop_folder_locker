...
use winapi::{
	shared::{
		...
		ntstatus::{
			STATUS_BUFFER_OVERFLOW, STATUS_INVALID_DEVICE_REQUEST, STATUS_NOT_IMPLEMENTED,
...
/// The context of a request: the one `create_file` returned for its handle,
/// or the handler's default for handles Dokany opened itself, which have
/// none (see [`FileSystemHandler::default_context`]).
fn context_of<'c, 'h: 'c, FSH: FileSystemHandler<'c, 'h> + 'h>(
	info: &OperationInfo<'c, 'h, FSH>,
) -> Option<&'c FSH::Context> {
	info.try_context()
		.or_else(|| info.handler().default_context())
}

/// [`context_of`], or returns `$status` from the enclosing closure.
macro_rules! context_or {
	($info:expr, $status:expr) => {
		match context_of(&$info) {
...
pub extern "stdcall" fn cleanup<'c, 'h: 'c, FSH: FileSystemHandler<'c, 'h> + 'h>(
	...
	wrap_unit(|| unsafe {
		...
		if let Some(context) = context_of(&info) {
...
pub extern "stdcall" fn read_file<'c, 'h: 'c, FSH: FileSystemHandler<'c, 'h> + 'h>(
	...
	wrap_nt_result(|| unsafe {
		*read_length = 0;
		...
		info.handler()
			.read_file(file_name, offset, buffer, &info, context_or!(info, STATUS_INVALID_DEVICE_REQUEST))
			.map(|bytes_read| {
...
pub extern "stdcall" fn write_file<'c, 'h: 'c, FSH: FileSystemHandler<'c, 'h> + 'h>(
	...
	wrap_nt_result(|| unsafe {
		*number_of_bytes_written = 0;
		...
		info.handler()
			.write_file(file_name, offset, buffer, &info, context_or!(info, STATUS_INVALID_DEVICE_REQUEST))
			.map(|bytes_written| {
...
pub extern "stdcall" fn flush_file_buffers<'c, 'h: 'c, FSH: FileSystemHandler<'c, 'h> + 'h>(
	...
	wrap_nt_result(|| unsafe {
		...
		let info = OperationInfo::<'c, 'h, FSH>::new(dokan_file_info);
		match context_of(&info) {
			Some(context) => info.handler().flush_file_buffers(file_name, &info, context),
			None => Ok(()),
		}
	})
}
...
pub extern "stdcall" fn get_file_information<'c, 'h: 'c, FSH: FileSystemHandler<'c, 'h> + 'h>(
	...
	wrap_nt_result(|| unsafe {
		...
		let info = OperationInfo::<'c, 'h, FSH>::new(dokan_file_info);
		info.handler()
			.get_file_information(file_name, &info, context_or!(info, STATUS_INVALID_DEVICE_REQUEST))
			.map(|file_info| {
...
pub extern "stdcall" fn find_files<'c, 'h: 'c, FSH: FileSystemHandler<'c, 'h> + 'h>(
	...
	wrap_nt_result(|| unsafe {
		...
		let info = OperationInfo::<'c, 'h, FSH>::new(dokan_file_info);
		info.handler()
			.find_files(file_name, fill_wrapper, &info, context_or!(info, STATUS_INVALID_DEVICE_REQUEST))
	})
}
...
pub extern "stdcall" fn find_files_with_pattern<'c, 'h: 'c, FSH: FileSystemHandler<'c, 'h> + 'h>(
	...
	wrap_nt_result(|| unsafe {
		...
		info.handler().find_files_with_pattern(
			...
			fill_wrapper,
			&info,
			context_or!(info, STATUS_INVALID_DEVICE_REQUEST),
		)
	})
}
...
pub extern "stdcall" fn set_file_attributes<'c, 'h: 'c, FSH: FileSystemHandler<'c, 'h> + 'h>(
	...
	wrap_nt_result(|| unsafe {
		...
		let info = OperationInfo::<'c, 'h, FSH>::new(dokan_file_info);
		info.handler()
			.set_file_attributes(file_name, file_attributes, &info, context_or!(info, STATUS_INVALID_DEVICE_REQUEST))
	})
}
...
pub extern "stdcall" fn delete_file<'c, 'h: 'c, FSH: FileSystemHandler<'c, 'h> + 'h>(
	...
	wrap_nt_result(|| unsafe {
		let file_name = U16CStr::from_ptr_str(file_name);
		let info = OperationInfo::<'c, 'h, FSH>::new(dokan_file_info);
		info.handler().delete_file(file_name, &info, context_or!(info, STATUS_INVALID_DEVICE_REQUEST))
	})
}
...
pub extern "stdcall" fn delete_directory<'c, 'h: 'c, FSH: FileSystemHandler<'c, 'h> + 'h>(
	...
	wrap_nt_result(|| unsafe {
		...
		let info = OperationInfo::<'c, 'h, FSH>::new(dokan_file_info);
		info.handler()
			.delete_directory(file_name, &info, context_or!(info, STATUS_INVALID_DEVICE_REQUEST))
	})
}
...
pub extern "stdcall" fn set_end_of_file<'c, 'h: 'c, FSH: FileSystemHandler<'c, 'h> + 'h>(
	...
	wrap_nt_result(|| unsafe {
		...
		let info = OperationInfo::<'c, 'h, FSH>::new(dokan_file_info);
		info.handler()
			.set_end_of_file(file_name, byte_offset, &info, context_or!(info, STATUS_INVALID_DEVICE_REQUEST))
	})
}
...
pub extern "stdcall" fn set_allocation_size<'c, 'h: 'c, FSH: FileSystemHandler<'c, 'h> + 'h>(
	...
	wrap_nt_result(|| unsafe {
		...
		let info = OperationInfo::<'c, 'h, FSH>::new(dokan_file_info);
		info.handler()
			.set_allocation_size(file_name, alloc_size, &info, context_or!(info, STATUS_INVALID_DEVICE_REQUEST))
	})
}
		...
		info.handler()
			.get_file_security(
				...
				*security_information,
				...
				buffer_length,
				&info,
				context_or!(info, STATUS_NOT_IMPLEMENTED),
			)
			.and_then(|needed| {
...
pub extern "stdcall" fn find_streams<'c, 'h: 'c, FSH: FileSystemHandler<'c, 'h> + 'h>(
	...
	wrap_nt_result(|| unsafe {
		...
		let info = OperationInfo::<'c, 'h, FSH>::new(dokan_file_info);
		info.handler()
			.find_streams(file_name, fill_wrapper, &info, context_or!(info, STATUS_NOT_IMPLEMENTED))
	})
}
