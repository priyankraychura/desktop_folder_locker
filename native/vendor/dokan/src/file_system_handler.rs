...
pub trait FileSystemHandler<'c, 'h: 'c>: Sync + Sized + 'h {
	...
	/// Context for requests on handles that [`create_file`] didn't open,
	/// which Dokany sends too (for example on the volume itself).
	///
	/// Without one, such requests fail with `STATUS_INVALID_DEVICE_REQUEST`
	/// (`cleanup` and `close_file` are skipped).
	///
	/// [`create_file`]: Self::create_file
