#![cfg(windows)]
// The callbacks must stay `extern "stdcall"` to match the types in
// dokan-sys; on 64-bit Windows that means the C ABI (see PATCHES.md).
#![allow(unknown_lints, unsupported_calling_conventions)]
