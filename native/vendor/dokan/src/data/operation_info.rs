...
impl<'c, 'h: 'c, FSH: FileSystemHandler<'c, 'h> + 'h> OperationInfo<'c, 'h, FSH> {
	...
	/// Gets the context `create_file` returned for the handle, or `None`
	/// for a handle it didn't open: Dokany also sends requests for handles
	/// it opened itself (for example on the volume), with no context.
	pub fn try_context(&self) -> Option<&'c FSH::Context> {
		unsafe { (self.file_info().Context as *const FSH::Context).as_ref() }
	}
